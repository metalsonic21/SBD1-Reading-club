...
CREATE TABLE SJL_ciudades (
    ...
    CONSTRAINT ciudades_pk PRIMARY KEY (id,id_pais)
);
...
CREATE TABLE SJL_urbanizaciones (
    ...
    id_ciudad   INTEGER NOT NULL,
    id_pais   INTEGER NOT NULL,
    CONSTRAINT urbanizaciones_pk PRIMARY KEY (id,id_ciudad,id_pais)
);
...
CREATE TABLE SJL_calles (
    ...
    id_ciudad         INTEGER NOT NULL,
    id_pais           INTEGER NOT NULL,
    CONSTRAINT calles_pk PRIMARY KEY (id,id_urb,id_ciudad,id_pais)
);
...
ALTER TABLE SJL_membresias ADD CONSTRAINT membresias_lectores_fk FOREIGN KEY(id_lec) REFERENCES SJL_lectores(doc_iden);
...
ALTER TABLE SJL_urbanizaciones ADD CONSTRAINT urb_ciudades_fk FOREIGN KEY(id_ciudad,id_pais) REFERENCES SJL_ciudades(id,id_pais);


ALTER TABLE SJL_calles ADD CONSTRAINT calles_urb_fk FOREIGN KEY(id_urb,id_ciudad,id_pais) REFERENCES SJL_urbanizaciones(id,id_urb,id_ciudad,id_pais);

ALTER TABLE SJL_telefonos ADD CONSTRAINT telefonos_lectores_fk FOREIGN KEY(id_lector) REFERENCES SJL_lectores(doc_iden);
...
CREATE SEQUENCE IF NOT EXISTS id_urbanizacion INCREMENT BY 1 MINVALUE 1 NO MAXVALUE START WITH 1 OWNED BY sjl_urbanizaciones.id;
